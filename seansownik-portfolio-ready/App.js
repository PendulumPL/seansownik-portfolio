...
import { Alert, AppState, Keyboard, KeyboardAvoidingView, Modal, Platform, Pressable, SafeAreaView, ScrollView, Share, StatusBar as NativeStatusBar, StyleSheet, Text, TextInput, View } from "react-native";
...
export default function App() {
  ...
  const exportBackup = async () => {
    try {
      await Share.share({
        title: "Kopia Seansownika",
        message: JSON.stringify({ version: 1, exportedAt: new Date().toISOString(), items }, null, 2)
      });
    } catch {
      Alert.alert("Nie udało się utworzyć kopii", "Spróbuj ponownie za chwilę.");
    }
  };
  ...
  const visible = useMemo(() => {
    ...
    return items.filter((item) => item.status === currentStatus && (platform === "Wszystkie" || item.platform === platform) && (!term || item.title.toLocaleLowerCase("pl").includes(term))).sort((a, b) => (b.updatedAt || b.createdAt || 0) - (a.updatedAt || a.createdAt || 0));
  ...
  const addManual = () => {
    ...
    if (!title) return;
    openEditor({ id: "manual-" + Date.now(), title, type: "Film / serial", year: "", info: "Dodano ręcznie", createdAt: Date.now() });
  };
  const save = () => {
    ...
    const now = Date.now();
    const saved = { ...editor, title: finalTitle, status: statusDraft, platform: platformDraft, rating: ratingDraft || null, note: noteDraft.trim(), createdAt: editor.createdAt || now, updatedAt: now, info: editor.info || "Dodano z bazy TMDB" };
  ...
  return <SafeAreaView style={styles.safe}>
    ...
    <KeyboardAvoidingView style={styles.app} behavior={Platform.OS === "ios" ? "padding" : undefined}>
      <View style={[styles.top, { paddingTop: 14 + androidTopInset }]}>
        <View style={styles.header}><View><Text style={styles.eyebrow}>{tab === "list" ? "TWOJA PÓŹNIEJSZA LISTA" : "DOBRY WIECZÓR"}</Text><Text style={styles.heading}>{tab === "list" ? "Moja lista" : "Co oglądamy?"}</Text></View><Pressable onPress={exportBackup} accessibilityLabel="Eksportuj kopię listy" style={{ width: 42, height: 42, borderRadius: 14, backgroundColor: "#fff", alignItems: "center", justifyContent: "center" }}><Ionicons name="share-outline" size={20} color="#171923" /></Pressable></View>
