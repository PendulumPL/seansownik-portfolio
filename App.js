...
import { Alert, AppState, Keyboard, KeyboardAvoidingView, Linking, Modal, Platform, Pressable, SafeAreaView, ScrollView, StatusBar as NativeStatusBar, StyleSheet, Text, TextInput, View } from "react-native";
...
import TmdbLogo from "./src/TmdbLogo";
...
export default function App() {
  ...
  return <SafeAreaView style={styles.safe}>
    ...
    <Modal visible={welcome} animationType="fade" statusBarTranslucent><View style={styles.welcome}>
      ...
      <View><Pressable onPress={dismissWelcome} style={styles.welcomeButton}><Text style={styles.welcomeButtonText}>Zaczynamy</Text><Ionicons name="arrow-forward" size={18} color="#fff" /></Pressable><Text style={styles.myszo}>✦ Autor: Paweł Karolak</Text></View>
    </View></Modal>
    ...
    <KeyboardAvoidingView style={styles.app} behavior={Platform.OS === "ios" ? "padding" : undefined}>
      ...
      <ScrollView style={styles.body} contentContainerStyle={styles.bodyContent} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
        ...
        <Pressable onPress={() => setAbout(true)} accessibilityRole="button" accessibilityLabel="O aplikacji i prywatność" style={styles.aboutLink}><Ionicons name="information-circle-outline" size={16} color="#717784" /><Text style={styles.aboutLinkText}>O aplikacji, prywatność i źródła danych</Text></Pressable>
      </ScrollView>
    ...
    <Modal visible={about} animationType="slide" onRequestClose={() => setAbout(false)} statusBarTranslucent navigationBarTranslucent>
      <View style={[styles.aboutModal, { paddingTop: androidTopInset, paddingBottom: androidBottomInset }]}><StatusBar style="light" />
        <ScrollView contentContainerStyle={styles.aboutContent} showsVerticalScrollIndicator={false}>
          <View style={styles.aboutHeader}><View><Text style={styles.aboutEyebrow}>INFORMACJE</Text><Text style={styles.aboutTitle}>O Seansowniku</Text></View><Pressable onPress={() => setAbout(false)} accessibilityLabel="Zamknij informacje" style={styles.aboutClose}><Ionicons name="close" size={22} color="#fff" /></Pressable></View>
          <Text style={styles.aboutVersion}>Wersja 0.2.1</Text>
          <View style={styles.aboutCard}><Text style={styles.aboutCardTitle}>Twoje dane</Text><Text style={styles.aboutText}>Lista filmów, oceny i notatki są zapisywane lokalnie na Twoim urządzeniu. Seansownik nie wymaga konta i nie wysyła Twojej listy na serwer autora aplikacji.</Text><Text style={styles.aboutText}>Kopia danych powstaje lub jest wczytywana tylko wtedy, gdy sam wybierzesz eksport albo import.</Text></View>
          <View style={styles.aboutCard}><Text style={styles.aboutCardTitle}>Wyszukiwanie i plakaty</Text><Text style={styles.aboutText}>Po wpisaniu tytułu zapytanie trafia do API The Movie Database (TMDB). Plakaty i logotypy platform są pobierane z serwerów TMDB, dlatego wyszukiwanie wymaga internetu.</Text></View>
          <View style={styles.tmdbCard}><TmdbLogo width={160} /><Text style={styles.tmdbNotice}>This product uses the TMDB API but is not endorsed or certified by TMDB.</Text><Pressable onPress={() => Linking.openURL("https://www.themoviedb.org/")} accessibilityRole="link"><Text style={styles.tmdbLink}>Otwórz stronę TMDB</Text></Pressable></View>
          <View style={styles.aboutCard}><Text style={styles.aboutCardTitle}>Autor i kontakt</Text><Text style={styles.aboutText}>Paweł Karolak · GitHub: PendulumPL</Text><Text style={styles.aboutText}>przywrocwspomnienia@gmail.com</Text></View>
        </ScrollView>
      </View>
    </Modal>
...
const styles = StyleSheet.create({
  aboutLink:{marginTop:16,alignSelf:"center",flexDirection:"row",alignItems:"center",gap:6,paddingVertical:8,paddingHorizontal:10},
  aboutLinkText:{color:"#717784",fontSize:10,fontWeight:"700",textDecorationLine:"underline"},
  aboutModal:{flex:1,backgroundColor:"#191B24"},
  aboutContent:{padding:20,paddingBottom:38},
  aboutHeader:{flexDirection:"row",alignItems:"center",justifyContent:"space-between",marginBottom:4},
  aboutEyebrow:{fontSize:10,letterSpacing:1.2,fontWeight:"900",color:"#9298A6",marginBottom:4},
  aboutTitle:{fontSize:28,fontWeight:"900",color:"#fff",letterSpacing:-0.7},
  aboutClose:{width:42,height:42,borderRadius:14,backgroundColor:"#2A2D38",alignItems:"center",justifyContent:"center"},
  aboutVersion:{color:"#9298A6",fontSize:12,fontWeight:"700",marginBottom:20},
  aboutCard:{backgroundColor:"#252833",borderWidth:1,borderColor:"#3C414E",borderRadius:17,padding:16,marginBottom:12},
  aboutCardTitle:{color:"#fff",fontSize:15,fontWeight:"900",marginBottom:8},
  aboutText:{color:"#CED2DC",fontSize:12,lineHeight:19,fontWeight:"600",marginBottom:7},
  tmdbCard:{backgroundColor:"#0D253F",borderRadius:17,padding:18,alignItems:"center",marginBottom:12},
  tmdbNotice:{color:"#fff",fontSize:11,lineHeight:17,textAlign:"center",fontWeight:"600",marginTop:12},
  tmdbLink:{color:"#01B4E4",fontSize:12,fontWeight:"900",textDecorationLine:"underline",marginTop:12},  watchNumber:{width:28,height:28,borderRadius:14,backgroundColor:"#EEF0F5",alignItems:"center",justifyContent:"center"},
